...
def main():
    x0, x_min, x_max, y_min, y_max, alpha_left, alpha_right, dot_num, figsize, levels, calc_epsilon, level_max_diff, \
        ...
        grads_color = \
        [4.0, 4.0], -0.5, 4.5, -3.0, 5.0, 0.0, 10.0, 500, (15, 7.5), [], 1e-2, 5, [2.0, 1.0], "ko", "k-", "ro", \
        0.25, 1e-2, "r"
    plt.figure(figsize=figsize)
    points_seq, grads_seq = nlopt.r_algorithm(f, x0, form='B', calc_epsilon=calc_epsilon, iter_lim=100, step_method='adaptive',
                                   default_step=10, step_red_mult=0.05, step_incr_mult=2, lim_num=3,
                                              reduction_epsilon=1e-15, return_grads=True)
    points_seq2 = nlopt.r_algorithm(f, x0, form='B', calc_epsilon=calc_epsilon, iter_lim=100, step_method='reduction',
