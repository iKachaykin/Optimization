...
if __name__ == '__main__':
    ...
    x0 = np.array([-1.2, -3.0])
    args = 0.0
    grad = lambda x, func, epsilon: np.array([400.0 * (x[0] ** 2 - x[1]) * x[0] + 2.0 * (x[0] - 1.0),
                                              -200.0 * (x[0] ** 2 - x[1])])
    form = 'B'
    beta = 1 / 3
    target = 'min'
    ...
    print('Отклонение от точного решения по переменной: %.16f' % np.linalg.norm(numerical_solution - exact_solution))
    print('Отклонение от точного решения по функции: %.16f' %
    ...
    if numerical_solution.size == 2:
        ...
        points_seq = points.copy()
        points_seq_for_levels = nlopt.remove_nearly_same_points(points, min_dist_between_points)
        grads_seq = []
        for point in points_seq:
            grads_seq.append(grad(point, lambda x: f(x, args), grad_epsilon))
        grads_seq = np.array(grads_seq)
        levels = np.sort(f([points_seq_for_levels[:, 0], points_seq_for_levels[:, 1]], args))
        count = 0
