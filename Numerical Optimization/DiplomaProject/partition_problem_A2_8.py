...
if __name__ == '__main__':
    ...
    x_left, x_right, y_left, y_right, grid_dot_num_x, grid_dot_num_y = -1.0, 1.0, -1.0, 1.0, 200, 200
    ...
    density_vector = [lambda x, y: np.where(np.logical_or(
        np.logical_and(np.logical_and(x >= x_left, x <= (x_left + x_right) / 2.0), np.logical_and(y >= -x-1, y <= x+1)),
        np.logical_and(np.logical_and(x >= (x_left + x_right) / 2.0, x <= x_right), np.logical_and(y >= x-1, y <= -x+1))
    ), 1.0, 0.0)]
    ...
    tau_initial[1] = np.random.rand(partition_number) * \
                     np.where(tau_initial[0] <= (x_left + x_right) / 2.0,
                              2 * tau_initial[0] + 2,
                              -2 * tau_initial[0] + 2) + \
                     np.where(tau_initial[0] <= (x_left + x_right) / 2.0, -tau_initial[0] - 1, tau_initial[0]-1)
    ...
    grid_dot_num_x_plotting, grid_dot_num_y_plotting = 1001, 1001
