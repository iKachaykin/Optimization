...
if __name__ == '__main__':
    ...
    Y_initial = np.zeros((partition_number, product_number))
    ...
    for product in range(product_number):
        ...
        for partition in range(partition_number):
            if (indicators_initial[partition, product] != 0).any():
                plt.contour(x_plotting, y_plotting, indicators_initial[partition, product], levels=[partition+1],
    ...
    r_alg_results = nlopt.r_algorithm_cooperative(
        ...
        form='H', beta=1/3, calc_epsilon_x=1e-4, calc_epsilon_grad=1e-4, iter_lim=1000, print_iter_index=True,
    ...
    for product in range(product_number):
        ...
        for partition in range(partition_number):
            if (indicators_solution[partition, product] != 0).any():
                plt.contour(x_plotting, y_plotting, indicators_solution[partition, product], levels=[partition+1],
                            cmap=ListedColormap(['black']))
                in_partition.append(partition)
        in_partition = np.array(in_partition)
