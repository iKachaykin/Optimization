...
def f(x, args):
    inds = np.arange(10) + 1
    return (10 ** -3) * ((1000.0 * np.exp(-0.2 * inds) + 2000.0 * np.exp(-0.4 * inds) -
                          x[0] * np.exp(-0.2 * x[1] * inds) - x[2] * np.exp(-0.4 * x[3] * inds)) ** 2).sum()
...
if __name__ == '__main__':
    ...
    inds = np.arange(10) + 1
    exact_solution = np.array([1000.0, 1.0, 2000.0, 1.0])
    x0 = np.array([500.0, 0.0, 2500.0, 3.0])
    args = 0.0
