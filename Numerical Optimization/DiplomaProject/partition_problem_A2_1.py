...
if __name__ == '__main__':
    ...
    density_vector = [lambda x, y: np.where(np.logical_and(np.logical_and(x >= 0, x <= 1/3),
                                                           np.logical_and(y >= 1/3, y <= 2/3)), 100.0, 1.0)]
    ...
    tau_initial = np.array([
        [0.17343, 0.27051, 0.07237],
        [0.61329, 0.42629, 0.42891]
    ])

    tau_initial += np.random.rand(2, partition_number) * 0.3
